...
class LabelFrame(tk.Frame):
    ...
    def __create_widgets(self):
        self.main_label = tk.Label(self, text="Welcome! Please select an option.", width=30).grid(column=0, row=0, sticky=tk.NSEW)
...
class ListBoxFrame(tk.Frame):
    ...
    def __create_widgets(self, container):
        ...
        self.lb = tk.Listbox(container, height=10, width=15, listvariable=self.content, yscrollcommand=self.sb.set).grid(row=1, column=0) # lb = List Box
